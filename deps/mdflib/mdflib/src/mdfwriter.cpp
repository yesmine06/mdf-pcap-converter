...
void MdfWriter::SaveCanMessage(const IChannelGroup& group, uint64_t time,
  ...
  } else if (group.Name() == "LIN_DataFrame" || group.Name() == "FlexRay_DataFrame" || group.Name() == "Ethernet_DataFrame") {
    /* LIN, FlexRay, Ethernet: meme format binaire que CAN_DataFrame (id, dlc, data) */
    msg.ToRaw(MessageType::CAN_DataFrame, sample, MaxLength(), save_index);
...
bool MdfWriter::CreateBusLogConfiguration() {
  ...
  switch (BusType()) {
    case MdfBusType::CAN:
      ...
      break;
    case MdfBusType::LIN:
      CreateLinConfig(*last_dg);
      break;
    case MdfBusType::FlexRay:
      CreateFlexRayConfig(*last_dg);
      break;
    case MdfBusType::Ethernet:
      CreateEthernetConfig(*last_dg);
      break;
    default:
...
void MdfWriter::CreateLinConfig(IDataGroup& dg_block) const {
  auto* cg_data_frame = dg_block.CreateChannelGroup("LIN_DataFrame");
  const IChannel* cn_data_byte = nullptr;
  if (cg_data_frame != nullptr) {
    cg_data_frame->PathSeparator('.');
    cg_data_frame->Flags(CgFlag::PlainBusEvent | CgFlag::BusEvent);
    CreateTimeChannel(*cg_data_frame, "t");
    CreateBusDataFrameChannel(*cg_data_frame, "LIN_DataFrame");
    cn_data_byte = cg_data_frame->GetChannel("LIN_DataFrame.DataBytes");
  }
  if (StorageType() == MdfStorageType::VlsdStorage && cn_data_byte != nullptr) {
    auto* cg_samples = dg_block.CreateChannelGroup("");
    if (cg_samples != nullptr) {
...
void MdfWriter::CreateFlexRayConfig(IDataGroup& dg_block) const {
  auto* cg_data_frame = dg_block.CreateChannelGroup("FlexRay_DataFrame");
  const IChannel* cn_data_byte = nullptr;
  if (cg_data_frame != nullptr) {
    cg_data_frame->PathSeparator('.');
    cg_data_frame->Flags(CgFlag::PlainBusEvent | CgFlag::BusEvent);
    CreateTimeChannel(*cg_data_frame, "t");
    CreateBusDataFrameChannel(*cg_data_frame, "FlexRay_DataFrame");
    cn_data_byte = cg_data_frame->GetChannel("FlexRay_DataFrame.DataBytes");
  }
  if (StorageType() == MdfStorageType::VlsdStorage && cn_data_byte != nullptr) {
    auto* cg_samples = dg_block.CreateChannelGroup("");
    if (cg_samples != nullptr) {
...
void MdfWriter::CreateEthernetConfig(IDataGroup& dg_block) const {
  auto* cg_data_frame = dg_block.CreateChannelGroup("Ethernet_DataFrame");
  const IChannel* cn_data_byte = nullptr;
  if (cg_data_frame != nullptr) {
    cg_data_frame->PathSeparator('.');
    cg_data_frame->Flags(CgFlag::PlainBusEvent | CgFlag::BusEvent);
    CreateTimeChannel(*cg_data_frame, "t");
    CreateBusDataFrameChannel(*cg_data_frame, "Ethernet_DataFrame");
    cn_data_byte = cg_data_frame->GetChannel("Ethernet_DataFrame.DataBytes");
  }
  if (StorageType() == MdfStorageType::VlsdStorage && cn_data_byte != nullptr) {
    auto* cg_samples = dg_block.CreateChannelGroup("");
    if (cg_samples != nullptr) {
...
void MdfWriter::CreateBusDataFrameChannel(IChannelGroup& group, const std::string& prefix) const {
  auto* cn_data_frame = group.CreateChannel(prefix);
  if (cn_data_frame == nullptr) return;
  ...
  switch (StorageType()) {
    case MdfStorageType::MlsdStorage:
      ...
      break;
    default:
      cn_data_frame->DataBytes(6 + 8);
      break;
  }
  cn_data_frame->Flags(CnFlag::BusEvent);
  cn_data_frame->DataType(ChannelDataType::ByteArray);

  const std::string busCh = prefix + ".BusChannel";
  const std::string idCh = prefix + ".ID";
  const std::string dlcCh = prefix + ".DLC";
  const std::string lenCh = prefix + ".DataLength";
  const std::string dataCh = prefix + ".DataBytes";

  auto* frame_bus = cn_data_frame->CreateChannelComposition(busCh);
  ...
  auto* frame_id = cn_data_frame->CreateChannelComposition(idCh);
  if (frame_id != nullptr) {
  ...
  auto* frame_dlc = cn_data_frame->CreateChannelComposition(dlcCh);
  ...
  auto* frame_length = cn_data_frame->CreateChannelComposition(lenCh);
  ...
  auto* frame_bytes = cn_data_frame->CreateChannelComposition(dataCh);
